...
async def get_trainer_intelligence(
    ...
    assigned_counts = dict((await session.execute(m_count_stmt)).all())

    # Revenue generated by trainer's assigned members
    rev_stmt = (
    ...
    for t in trainers:
        ...
        trainer_items.append(
            TrainerPerformanceItem(
                ...
                retention_rate=round(85.0 + (t.rating - 4.5) * 15.0, 1) if t.rating >= 4.0 else 75.0,
            )
        )
